  ...
  <body>
	...
	<div class="admin_body">
		<ul class="admin_ul">
			...
			<li><a href="ordersListPaging?currentPage=1" class="active">竞拍管理</a></li>
		...
		<div class="admin_section">
			<div class="admin_section_box">
				<div class="admin_section_box_content">
					...
					<div>
						<form action="ordersQuery" method="get">
							<span>
					...
					<table align="center" border="0" cellpadding="0" cellspacing="0">
						...
						<s:iterator value="#request['orderss']" var="a_orders">
							<tr align="center" height="26px" class="jquery_tr">
								<td>${a_orders.id}</td>
								<td class="orders_content">${a_orders.goods.title}</td>
								<td>${a_orders.user.username}</td>
								<td>${a_orders.createat}</td>
								<td>${a_orders.jprice}</td>
								<td>${a_orders.fkstatus}</td>
							</tr>
						</s:iterator>
					</table>
				</div>
				...
				<div class="paging_pager">
					<div class="paging_pager_bar">
						...
						<s:if test="#pager.hasFirst">
						  <a class="pager_commom" href="ordersListPaging.action?currentPage=1">首页</a>
						</s:if>
						<s:else>
						...
						<s:if test="#pager.hasPrevious">
							<a class="pager_commom" href="ordersListPaging.action?currentPage=<s:property value="#pager.currentPage-1"/>">&lsaquo;</a>
						</s:if>
						<s:else>
						...
						<!-- 当前页 -->
						<a class="pager_commom" href="ordersListPaging.action?currentPage=<s:property value="#pager.currentPage"/>"><s:property value="#pager.currentPage"/></a>
						
						<!-- 下一页 -->
						<s:if test="#pager.hasNext">
						  <a class="pager_commom" href="ordersListPaging.action?currentPage=<s:property value="#pager.currentPage+1"/>">&rsaquo;</a>
						</s:if>
						<s:else>
						...
						<s:if test="#pager.hasLast">
						  <a class="pager_commom" href="ordersListPaging.action?currentPage=<s:property value="#pager.totalPage"/>">尾页</a>
						</s:if>
						<s:else>
